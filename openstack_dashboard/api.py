...
In other words, openstack_dashboard developers not working on openstack_dashboard.api
...
def check_openstackx(f):
    """Decorator that adds extra info to api exceptions
       ...
       The openstack_dashboard currently depends on openstackx extensions being present
       ...
       openstack_dashboard more gracefully handle extensions and openstackx extensions
       aren't required by the openstack_dashboard in nova.
    """
    def inner(*args, **kwargs):
        try:
            ...
            e.message += ' This error may be caused by missing openstackx' \
                         ' extensions in nova. See the openstack_dashboard README.'
            raise
...
def server_create(request, name, image, flavor, key_name, user_data, security_groups):
